...
`asyncfsm` can be used both as a Rust library in your projects and as a standalone command-line tool.

## Library Usage

### Installation

Add `asyncfsm` to your `Cargo.toml`:

```toml
[dependencies]
asyncfsm = { git = "https://github.com/vvr/asyncfsm.git" }
...
```toml
[dependencies]
asyncfsm = { git = "...", default-features = false, features = ["clitable", "yaml"] }
```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 1. Load the template
    let mut fsm = TextFSM::from_file("path/to/template.asyncfsm")?;
...
The `asyncfsm` binary allows you to parse files directly from the terminal.
...
# OR
cargo install asyncfsm # If published to crates.io (future)
```
...
**Usage:**
```bash
asyncfsm parse --template <TEMPLATE_PATH> --input <DATA_PATH> [--lowercase] [--format <json|yaml|csv|text|html|xml>]
```

**Example:**
```bash
asyncfsm parse \
  --template templates/cisco_ios_show_version.asyncfsm \
